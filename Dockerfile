...
RUN pacman -Sy --noconfirm --needed binutils openssl libffi
RUN pacman -Sy --noconfirm --needed zsh git wget vim nano gdb python python-pip nmap nikto wfuzz dirb openssh sshpass openvpn metasploit exploitdb mitmproxy binwalk
RUN pacman -Sy --noconfirm --needed net-tools zip unrar sslsplit sqlmap john hydra padbuster stegdetect stegsolve steghide perl-image-exiftool masscan xsser 
