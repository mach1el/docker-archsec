...
RUN pacman -Sy --noconfirm --needed zsh git wget vim nano gdb python python-pip binutils openssl libffi python2-paramiko python-paramiko postgresql mlocate
RUN pacman -Sy --noconfirm --needed nmap nikto wfuzz dirb openssh sshpass openvpn metasploit exploitdb mitmproxy binwalk seclists gnu-netcat
